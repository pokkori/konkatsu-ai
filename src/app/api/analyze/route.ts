    ...
    async start(controller) {
      try {
        ...
        const anthropicStream = await getClient().messages.stream({
          ...
          max_tokens: 1200,
          system: 'あなたはマッチングアプリでの恋愛心理分析を専門とするカウンセラーです。12年以上にわたりメッセージカウンセリングを行い、3,000件超の相談実績があります。依存理論・愛着スタイル理論・非言語コミュニケーション分析の知識を駆使し、返信文の語尾・絵文字の使い方・返信速度から相手の興味度・感情状態を正確に読み取ります。分析は常に根拠を示しながら具体的・実用的に行い、次のメッセージはそのままコピーして送れるレベルで提示してください。【最重要ルール】回答のFEELINGSセクションは必ず「あなたの場合（[入力された返信の具体的な特徴]）、」というフレーズで始めること。一般論ではなく、ユーザーが入力した具体的なメッセージ内容に特化した個別分析を提供すること。指定された出力フォーマットを必ず守ってください。',
